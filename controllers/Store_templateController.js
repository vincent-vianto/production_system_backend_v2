const Store_templates = require('../models/Store_template')

const getAllFormNumber = async (req, res) => {
	const store_template = await Store_templates.find().select('form_number name')
	if (!store_template)
		return res.status(204).json({ message: 'No template found' })
	res.json(store_template)
}

const addForm = async (req, res) => {
	try {
		const result = await Store_templates.create({
			name: req.body.name,
...
const getFormByFormNumber = async (req, res) => {
	...
	const form = await Store_templates.findOne({
